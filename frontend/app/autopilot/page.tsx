...
interface AutopilotSession {
  ...
  topic_mastery: Record<
    string,
    { topic: string; score: number; attempts: number }
  >;
  steps: AutopilotStep[];
...
const ACTION_ICONS: Record<string, React.ReactNode> = {
  session_started: (
    <HugeiconsIcon icon={Play} size={16} className="text-primary" />
  ),
  topic_selected: (
    <HugeiconsIcon icon={Target} size={16} className="text-chart-1" />
  ),
  lesson_started: (
    <HugeiconsIcon icon={Book02Icon} size={16} className="text-chart-2" />
  ),
  lesson_completed: (
    <HugeiconsIcon
  ...
  quiz_generated: (
    <HugeiconsIcon icon={Brain} size={16} className="text-chart-3" />
  ),
  answer_evaluated: (
  ...
  misconception_busted: (
    <HugeiconsIcon icon={Zap} size={16} className="text-chart-3" />
  ),
  topic_completed: (
    <HugeiconsIcon
      icon={CheckmarkCircle02Icon}
  ...
  plan_updated: (
    <HugeiconsIcon icon={Refresh01Icon} size={16} className="text-primary" />
  ),
  session_paused: (
    <HugeiconsIcon icon={Pause} size={16} className="text-muted-foreground" />
  ),
  session_completed: (
    <HugeiconsIcon
      icon={CheckmarkCircle02Icon}
  ...
  self_correction: (
    <HugeiconsIcon icon={Refresh01Icon} size={16} className="text-chart-3" />
  ),
};
...
export default function AutopilotPage() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setSession((prev) => {
          ...
          if (
            JSON.stringify(prev.current_question) !==
              JSON.stringify(data.current_question) ||
  ...
  const handlePause = async () => {
    try {
      await fetch(`${API_BASE}/api/autopilot/pause/${sessionId}`, {
        method: "POST",
      });
      await pollStatus();
    } catch (e) {
  ...
  const handleResume = async () => {
    try {
      await fetch(`${API_BASE}/api/autopilot/resume/${sessionId}`, {
        method: "POST",
      });
      await pollStatus();
    } catch (e) {
  ...
  const handleStop = async () => {
    try {
      await fetch(`${API_BASE}/api/autopilot/stop/${sessionId}`, {
        method: "POST",
      });
      if (pollingRef.current) clearInterval(pollingRef.current);
      await pollStatus();
          ...
          <div className="grid md:grid-cols-3 gap-4 mb-10 text-left">
            <Card className="p-5 border border-border bg-card/50 hover:border-primary/30 transition-colors backdrop-blur-sm">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Dynamically chooses what to study next based on your mastery
                gaps.
              </p>
            </Card>
            <Card className="p-5 border border-border bg-card/50 hover:border-chart-2/30 transition-colors backdrop-blur-sm">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Streams concise, 2-minute explanations tailored to you.
              </p>
            </Card>
            <Card className="p-5 border border-border bg-card/50 hover:border-chart-3/30 transition-colors backdrop-blur-sm">
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                Detects mistakes and pauses to fix misconceptions instantly.
              </p>
            </Card>
          </div>
          ...
          {!hasPlan ? (
            ...
            <Button onClick={handleStart} disabled={loading} size="md">
              {loading ? (
                ...
                <>
                  Start Session{" "}
                  <HugeiconsIcon icon={Play} size={18} className="ml-2" />
                </>
              )}
            </Button>
          )}
          ...
          <div className="flex items-center gap-3">
            ...
            {session.status === "running" ? (
              <Button size="sm" variant="outline" onClick={handlePause}>
                <HugeiconsIcon icon={Pause} size={14} className="mr-1" /> Pause
              </Button>
            ) : (
              <Button size="sm" onClick={handleResume}>
                <HugeiconsIcon icon={Play} size={14} className="mr-1" /> Resume
              </Button>
            )}
            <Button size="sm" variant="destructive" onClick={handleStop}>
              <HugeiconsIcon icon={Square} size={14} className="mr-1" /> Stop
            </Button>
          </div>
        </div>
          ...
          <Card className="p-4 mb-4 border border-border bg-card/60">
            ...
            <div className="flex gap-2 flex-wrap">
              {Object.entries(session.topic_mastery).map(([key, data]) => (
                <div
                  key={key}
                  className="px-3 py-1.5 rounded-md bg-primary/10 border border-primary/20 text-xs font-medium"
                >
                  <span className="text-foreground">{data.topic}:</span>{" "}
        ...
        <div className="pb-6">
          <div className="grid lg:grid-cols-12 gap-6 items-start">
            ...
            <div className="lg:col-span-8 flex flex-col gap-4">
              ...
              <Card className="flex-1 border border-border bg-card/60 relative overflow-hidden flex flex-col">
                ...
                <div className="absolute top-4 right-4 z-20">
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border backdrop-blur-md shadow-sm ${
                      session.current_phase === "teaching"
                        ? "bg-chart-2/20 border-chart-2/30 text-chart-2"
                        : session.current_phase === "quizzing"
                ...
                <div className="p-8 h-[75vh] overflow-y-auto">
                  {/* 1. TEACHING VIEW */}
                  {session.current_phase === "teaching" &&
                    session.current_content && (
                      <div className="max-w-3xl mx-auto animate-in fade-in duration-500">
                        ...
                        <h2 className="text-2xl font-bold mb-6">
                          Micro-Lesson: {session.current_topic}
                        </h2>
                    ...
                    session.current_question && (
                      <div className="max-w-2xl mx-auto animate-in zoom-in-95 duration-300">
                        ...
                        <div className="space-y-3">
                          {session.current_question.options.map(
                            (option, idx) => (
                              <button
                                key={idx}
                  ...
                  {!session.current_content && !session.current_question && (
                    <div className="h-full flex flex-col items-center justify-center text-center opacity-60">
                      ...
                      <p className="text-sm">
                        Analysing your progress & selecting next topic
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="lg:col-span-4">
              <Card className="h-[75vh] flex flex-col border border-border bg-card/60">
                ...
                <div
                  ref={runLogRef}
                  className="flex-1 overflow-y-auto min-h-0 px-4 pt-4 pb-4 space-y-4"
                >
                  {session.steps.map((step, index) => (
                    ...
                    >
                      <div className="absolute -left-[5px] top-1 w-2.5 h-2.5 rounded-full border border-border bg-background flex items-center justify-center">
                        <div
                          className={`w-1 h-1 rounded-full ${step.action.includes("misconception") ? "bg-destructive" : "bg-primary"}`}
                        ></div>
                      </div>
